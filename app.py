...
#  1. Replaced deprecated use_container_width=True  →  width='stretch'
#     (Streamlit ≥ 1.45 deprecation; removed after 2025-12-31)
#  2. Data fetching now handled by the improved data/fetcher.py which has
#     retry logic, exponential back-off, and 15-min Streamlit disk caching.
...
def render_screener(ticker: str, period: str):
    ...
    if df.empty:
        st.warning(
            ...
            "This sometimes happens due to Yahoo Finance rate limiting. "
            "Please wait 30 seconds and try again, or select a different stock."
        )
        st.info(
...
def render_analysis(ticker: str, period: str):
    ...
    # FIX: replaced use_container_width=True  →  width='stretch'
    st.plotly_chart(plot_smc_chart(df, smc, ticker), width="stretch")
